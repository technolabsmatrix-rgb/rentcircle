...
/* ─── Skeleton Loader ─── */
function SkeletonRows({ cols = "2fr 1fr 1fr 1fr 1fr", count = 6 }) {
  return (
    <>
...
export default function AdminPortal() {
  ...
  const { categories, loading: loadingCategories, save: saveCategory, remove: removeCategory } = useCategories(initialCategories);
  const { products, setProducts, loading: loadingProducts, add: addProduct, update: updateProductDb, remove: removeProduct, refresh: refreshProducts } = useProducts(initialProducts);
  const { plans, loading: loadingPlans, save: savePlanDb } = usePlans(initialPlans);
  const { users, setUsers, loading: loadingUsers, save: saveUserDb } = useProfiles(initialUsers);
  const { tags, setTags, loading: loadingTags, save: saveTagDb, remove: removeTagDb } = useTags(initialTags);
  const { flags: featureFlags, toggle: toggleFlag } = useFeatureFlags(defaultFlags);
  const { customFields, loading: loadingCustomFields, save: saveCustomFieldDb, remove: removeCustomFieldDb } = useCustomFields(initialCustomFields);
  const { orders, loading: loadingOrders } = useOrders(initialOrders);
    ...
    return (
      <>
        ...
        <div style={s.card}>
          ...
          {loadingProducts ? <SkeletonRows cols="2fr 1fr 0.9fr 0.7fr 0.7fr 1.5fr 1.2fr 1.4fr" count={6} /> : filteredProducts.length === 0 && <div style={{ padding: "3rem", textAlign: "center", color: COLORS.muted }}>No products match your filters</div>}
          {!loadingProducts && filteredProducts.slice((pPage-1)*pPerPage, pPage*pPerPage).map(p => {
            ...
            <div key={u.id} style={s.tr("1.8fr 1.6fr 0.7fr 0.6fr 1.8fr 0.7fr 1.5fr")} onMouseEnter={e => e.currentTarget.style.background = COLORS.surfaceHover} onMouseLeave={e => e.currentTarget.style.background = ""}>
              ...
              <div style={{ display: "flex", gap: "0.35rem" }}>
                ...
                <button style={{ ...s.btn("danger"), padding: "0.3rem 0.6rem", fontSize: "0.76rem" }}
                  onClick={async () => { const upd = { ...u, status: u.status === "active" ? "suspended" : "active" }; setUsers(prev => prev.map(x => x.id === u.id ? { ...x, status: upd.status } : x)); try { await saveUserDb(upd); showNotif(u.status === "active" ? "User suspended" : "User reactivated"); } catch(e) { setUsers(prev => prev.map(x => x.id === u.id ? { ...x, status: u.status } : x)); showNotif("Failed: " + e.message, "error"); } }}>
  ...
  const renderOrders = () => {
    ...
    // Normalize DB snake_case OR fallback camelCase fields
    const oName    = o => o.user_name      || o.user      || "—";
    const oEmail   = o => o.user_email     || o.userEmail  || "";
    const oStart   = o => o.start_date     || o.startDate  || "—";
    const oEnd     = o => o.end_date       || o.endDate    || "—";
    const oAddr    = o => o.delivery_address || "";
    const oPhone   = o => o.delivery_phone   || "";
    const oPayment = o => o.payment_method   || "";
    return (
      <>
        ...
        <div style={s.card}>
          ...
          <div style={s.th("0.9fr 1.8fr 1.6fr 0.5fr 1fr 1.05fr 1.05fr 1.8fr 1.4fr")}>
            ...
            <SortableCol label="Start" field="startDate" sortField={oSort} sortDir={oDir} onSort={(f,d) => { setOSort(f); setODir(d); }} />
            <SortableCol label="End" field="endDate" sortField={oSort} sortDir={oDir} onSort={(f,d) => { setOSort(f); setODir(d); }} />
            <span>Delivery</span>
            <span>Status</span>
          </div>
          {loadingOrders ? <SkeletonRows cols="0.9fr 1.8fr 1.6fr 0.5fr 1fr 1.05fr 1.05fr 1.8fr 1.4fr" count={6} /> : filteredOrders.length === 0 && <div style={{ padding: "3rem", textAlign: "center", color: COLORS.muted }}>No orders match your search</div>}
          {!loadingOrders && filteredOrders.slice((oPage-1)*oPerPage, oPage*oPerPage).map(order => {
            const orderUser = users.find(u => u.email === oEmail(order));
            const userName = oName(order);
            const deliveryAddr = oAddr(order);
            const deliveryPhone = oPhone(order);
            const paymentMethod = oPayment(order);
            return (
              <div key={order.id} style={{ borderBottom: `1px solid ${COLORS.border}` }}>
                {/* Main row */}
                <div style={{ display: "grid", gridTemplateColumns: "0.9fr 1.8fr 1.6fr 0.5fr 1fr 1.05fr 1.05fr 1.8fr 1.4fr", gap: "0.75rem", padding: "0.85rem 1.25rem", alignItems: "center" }} onMouseEnter={e => e.currentTarget.style.background = COLORS.surfaceHover} onMouseLeave={e => e.currentTarget.style.background = ""}>
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: "0.5rem", minWidth: 0 }}>
                    ...
                    <div style={{ minWidth: 0 }}>
                      <div style={{ fontSize: "0.82rem", fontWeight: 600, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{userName}</div>
                      {orderUser && <div style={{ fontSize: "0.68rem", color: COLORS.accent }}>{orderUser.plan} · {oEmail(order)}</div>}
                      {!orderUser && oEmail(order) && <div style={{ fontSize: "0.68rem", color: COLORS.muted, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{oEmail(order)}</div>}
                    </div>
                  </div>
                  <span style={{ fontSize: "0.85rem" }}>{order.days}d</span>
                  <span style={{ color: COLORS.green, fontWeight: 700 }}>{INR(order.amount)}</span>
                  <div>
                    <div style={{ fontSize: "0.78rem", color: COLORS.text, fontWeight: 500 }}>{oStart(order)}</div>
                  </div>
                  <div>
                    <div style={{ fontSize: "0.78rem", color: COLORS.text, fontWeight: 500 }}>{oEnd(order)}</div>
                  </div>
                  {/* Delivery address cell */}
                  <div style={{ minWidth: 0 }}>
                    {deliveryAddr ? (
                      <>
                        <div style={{ fontSize: "0.78rem", color: COLORS.text, fontWeight: 500, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }} title={deliveryAddr}>📍 {deliveryAddr}</div>
                        {deliveryPhone && <div style={{ fontSize: "0.68rem", color: COLORS.muted, marginTop: "0.1rem" }}>📞 {deliveryPhone}</div>}
                        {paymentMethod && <div style={{ fontSize: "0.65rem", color: COLORS.accent, fontWeight: 700, marginTop: "0.1rem", textTransform: "uppercase" }}>💵 {paymentMethod}</div>}
                      </>
                    ) : (
            ...
            <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.55)", zIndex: 400, display: "flex", justifyContent: "flex-end" }} onClick={() => setSelectedUser(null)}>
              <div style={{ width: "min(580px,100vw)", background: COLORS.surface, height: "100%", overflowY: "auto", boxShadow: "-8px 0 40px rgba(0,0,0,0.18)", animation: "slideInRight 0.25s ease" }} onClick={e => e.stopPropagation()}>
                ...
                <div style={{ padding: "1.5rem 1.75rem", display: "flex", flexDirection: "column", gap: "1.5rem" }}>
                  ...
                  {/* Profile info */}
                  <div>
                    <div style={{ fontWeight: 700, fontSize: "0.72rem", color: COLORS.muted, textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: "0.65rem" }}>👤 Profile</div>
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0.5rem" }}>
                      {[["📱 Phone", u.phone || "—"], ["🏙 City", u.city || "—"], ["📅 Joined", u.joined || "—"], ["🎟 Plan", u.plan || "—"]].map(([k, v]) => (
                        <div key={k} style={{ background: COLORS.bg, borderRadius: "8px", padding: "0.55rem 0.85rem", border: `1px solid ${COLORS.border}` }}>
                          <div style={{ fontSize: "0.67rem", color: COLORS.muted, fontWeight: 600 }}>{k}</div>
                          <div style={{ fontWeight: 700, fontSize: "0.88rem", marginTop: "0.15rem" }}>{v}</div>
                        </div>
                      ))}
                    </div>
                    <div style={{ display: "flex", gap: "0.5rem", marginTop: "0.5rem" }}>
                      {[["📧 Email", u.emailVerified], ["📱 Phone", u.phoneVerified]].map(([lbl, verified]) => (
                        <div key={lbl} style={{ flex: 1, background: verified ? "rgba(16,185,129,0.08)" : "rgba(239,68,68,0.08)", borderRadius: "8px", padding: "0.55rem 0.85rem", border: `1px solid ${verified ? "rgba(16,185,129,0.2)" : "rgba(239,68,68,0.2)"}` }}>
                          <div style={{ fontSize: "0.67rem", color: COLORS.muted, fontWeight: 600 }}>{lbl}</div>
                          <div style={{ fontWeight: 700, color: verified ? COLORS.green : COLORS.red, marginTop: "0.15rem" }}>{verified ? "✓ Verified" : "✗ Unverified"}</div>
                        </div>
                      ))}
                    </div>
                  </div>
                  ...
                  {/* As Renter */}
                  <div>
                    <div style={{ fontWeight: 700, fontSize: "0.72rem", color: COLORS.muted, textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: "0.65rem" }}>🛒 Rental Activity (as Renter)</div>
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: "0.6rem" }}>
                      <SBox icon="📦" label="Total Orders" value={rentedOrders.length} />
                  ...
                  <div>
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr", gap: "0.5rem" }}>
                      <SBox icon="📋" label="Orders Received" value={ownerOrders.length} />
                      <SBox icon="💵" label="Gross Revenue" value={INR(grossRevenue)} color={COLORS.blue} sub="100%" />
                      <SBox icon="🏦" label="Platform (30%)" value={INR(commission)} color={COLORS.red} sub="Commission" />
                      <SBox icon="✅" label="Net Earnings" value={INR(netRevenue)} color={COLORS.green} sub="70% to owner" />
                    </div>
                    {grossRevenue > 0 && (
                      <div style={{ marginTop: "0.65rem", background: COLORS.bg, borderRadius: "10px", padding: "0.85rem", border: `1px solid ${COLORS.border}` }}>
                        <div style={{ display: "flex", justifyContent: "space-between", fontSize: "0.73rem", fontWeight: 600, marginBottom: "0.4rem" }}>
                          <span style={{ color: COLORS.green }}>Owner 70% — {INR(netRevenue)}</span>
                          <span style={{ color: COLORS.red }}>Platform 30% — {INR(commission)}</span>
                        </div>
                        <div style={{ height: "9px", borderRadius: "5px", background: "rgba(239,68,68,0.15)", overflow: "hidden" }}>
                    ...
                    {ownerProducts.length > 0 && (
                      <div style={{ marginTop: "0.65rem" }}>
                        <div style={{ fontSize: "0.7rem", fontWeight: 700, color: COLORS.muted, marginBottom: "0.4rem" }}>🏷 Listed Products ({ownerProducts.length})</div>
